# Wapsell — Charter (la constitución del proyecto)
...
**Wapsell = SaaS de agentes de ventas automáticos por WhatsApp.**

Una empresa contrata Wapsell → su número de WhatsApp pasa a estar atendido 24/7
...
2. **Cero hardcoding de cliente.** Ningún nombre de empresa, dominio, número de
   ...
   `tenant.*` / `settings.*`. Si aparece "Wapsell SA" o "ENACOM" o un teléfono
...
5. **Gate verde antes de commit.** `ruff check . && ruff format --check . &&
   mypy sdk/wapsell services tests && pytest`. Sin excepciones. Si un cambio no
...
4. **Cuando Wapsell esté operativo — Extraer las 3 templates.**
...
- ❌ No mezclamos código de Wapsell con AINE (son proyectos separados).
- ❌ No subimos Wapsell a GitHub público hasta que esté productivo y auditado
  (puede llevar info de clientes; cuando se sube, va privado).

## Métricas de "está listo para producción"

Wapsell se considera **production-ready** cuando, en orden:
