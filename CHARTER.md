# Waseller — Charter (la constitución del proyecto)
...
**Waseller = SaaS de agentes de ventas automáticos por WhatsApp.**

Una empresa contrata Waseller → su número de WhatsApp pasa a estar atendido 24/7
...
2. **Cero hardcoding de cliente.** Ningún nombre de empresa, dominio, número de
   ...
   `tenant.*` / `settings.*`. Si aparece "Waseller SA" o "ENACOM" o un teléfono
...
5. **Gate verde antes de commit.** `ruff check . && ruff format --check . &&
   mypy sdk/waseller services tests && pytest`. Sin excepciones. Si un cambio no
...
4. **Cuando Waseller esté operativo — Extraer las 3 templates.**
...
- ❌ No mezclamos código de Waseller con HookClose (son proyectos separados).
- ❌ No subimos Waseller a GitHub público hasta que esté productivo y auditado
  (puede llevar info de clientes; cuando se sube, va privado).

## Métricas de "está listo para producción"

Waseller se considera **production-ready** cuando, en orden:
