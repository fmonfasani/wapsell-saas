...
## Objetivo
"Waseller en blanco": un emprendedor clona T3, conecta sus LLMs (OpenRouter)
y su número de Meta, y tiene un SaaS de ventas por WhatsApp funcionando bajo su
propia marca.

## Pre-requisito
- T2 mergeada y testeada (P92 done).
- Waseller production-ready (todas las fases verdes y deployada en una VPS al
...
3. Renombrar referencias a `waseller` → `{{ project_slug }}` en imports/strings.
...
## Reglas
- Cero referencias a "Waseller" (la marca) en T3. La marca vive en el cliente.
...
Waseller sigue siendo el "cliente cero" que valida T3 en producción.
